...
import { base_url, login_url,key_url } from "../../Config/Auth";
...
export const login = ( {emailId,otp} ,navigate,   cb) => (dispatch) => {
  dispatch({
    type: types.LOGIN_REQUEST,
  });
  axios
...
export const generateOtpByEmail = (data) => (dispatch) => {
  ...
  export const getUserDetails = (candidateId) => (dispatch) => {
    ...
    axios
      .get(`${base_url}/candidate/login/${candidateId}?url=${key_url}`, {
        // headers: {
  ...
  export const getHourListByUserId = (candidateId) => (dispatch) => {
    ...
    axios
      .get(`${base_url}/hour/user/planner/website/${candidateId}?url=${key_url}`, {
        // headers: {
